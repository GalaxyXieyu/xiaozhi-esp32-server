...
public class OTAController {
    ...
    public ResponseEntity<String> getOTA() {
        ...
        boolean hasMqttGateway = StringUtils.isNotBlank(mqttUdpConfig) && !"null".equals(mqttUdpConfig);
        boolean hasWebsocket = StringUtils.isNotBlank(wsUrl) && !"null".equals(wsUrl);
        if (!hasMqttGateway && !hasWebsocket) {
            return ResponseEntity.ok("OTA接口不正常，缺少mqtt_gateway或websocket地址，请登录智控台，在参数管理找到【server.mqtt_gateway】或【server.websocket】配置");
        }
        String otaUrl = sysParamsService.getValue(Constant.SERVER_OTA, true);
        if (StringUtils.isBlank(otaUrl) || otaUrl.equals("null")) {
            return ResponseEntity.ok("OTA接口不正常，缺少ota地址，请登录智控台，在参数管理找到【server.ota】配置");
        }
        int websocketClusterCount = hasWebsocket ? wsUrl.split(";").length : 0;
        return ResponseEntity.ok("OTA接口运行正常，websocket集群数量：" + websocketClusterCount);
    }
